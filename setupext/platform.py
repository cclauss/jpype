...
logger = utils.getLogger("jpype_setup.platform")
...
def Platform(include_dirs=None, sources=None, platform=sys.platform):
    ...
    if os.path.exists(java_home):
        ...
        for d in platform_specific['include_dirs']:
            if os.path.exists(os.path.join(str(d), 'jni.h')):
                logger.info("Found native jni.h at %s", d)
                found_jni = True
                break

        if not found_jni:
            logger.warning('Falling back to provided JNI headers, since your provided'
    ...
    logger.info("Configure platform to %s", platform)

    if platform == 'win32':
        logger.info("Add windows settings")
        ...
        logger.info("Add darwin settings")
        ...
        logger.info("Add zos settings")
        jni_md_platform = 'zos'

    elif platform == 'sunos5':
        logger.info("Add solaris settings")
        jni_md_platform = 'solaris'
        
    else:
        jni_md_platform = ''
        logger.warn("Your platform '%s' is not being handled explicitly."
...
#        platform_specific['extra_link_args'].append(sysconfig.get_config_var('BLDLIBRARY'))
    ...
    if found_jni:
        logger.info("Add JNI directory %s", os.path.join(java_home, 'include', jni_md_platform))
